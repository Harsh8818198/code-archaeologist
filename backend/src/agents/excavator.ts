...
export class ExcavatorAgent {
  ...
  private async buildKnowledgeGraph(
    ...
    for (const file of files) {
      ...
      for (const def of file.metrics.definitions.slice(0, 5)) {
        ...
        edges.push({
          ...
          label: "defines", relationship: "defines",
          weight: 1,
        });
      }
    }
      ...
      for (const file of files) {
        if (file.authors.includes(author.name)) {
          edges.push({
            ...
            label: "authored", relationship: "authored",
      ...
      for (const imp of file.metrics.imports) {
        ...
        if (matchingFile) {
          edges.push({
            ...
            label: "imports", relationship: "imports",
            weight: 0.8,
          });
        }
      }
    ...
    return { nodes, edges, clusters, entities: [], relationships: [] };
  }
